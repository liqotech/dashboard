...
export default function Utils() {
      ...
      Object.keys(schemaGen).forEach(key => {
        if (
          ...
          if (schemaGen[key].type) {
            if (
              schemaGen[key].type === 'object' ||
              schemaGen[key].type === 'array' ||
              !schemaGen[key].type
            ) {
  ...
  const replaceObject = (item, itemKey, origResource) => {
    Object.keys(origResource).forEach(key => {
      ...
      if (
        typeof origResource[key] === 'object' ||
        Array.isArray(origResource[key])
      )
  ...
  function setCookie(token, isOnLocal) {
    ...
    token.match(/(.|[\r\n]){1,4000}/g).forEach(chunk => {
      ...
      else Cookies.set('token_' + counter, chunk, props);
      counter++;
    });
  }
