...
export default function ApiInterface(_user, tokenLogout) {
  ...
  const setNamespace = _namespace => {
    if (_namespace === 'all namespaces') namespace.current = null;
  ...
  const handleError = error => {
    ...
    else if (error === 401 || error === 403) return Promise.reject(error);
    return Promise.reject(
      error.response ? error.response._fetchResponse.status : error
    );
  };
        ...
        .catch(error => {
          if (kind === 'DashboardConfig') {
            return getGenericResource(
              '/apis/dashboard.liqo.io/v1alpha1/dashboardconfigs/default-config'
            )
              .then(res => {
    ...
    if (enabledConfigs) {
      ...
      enabledConfigs.forEach(config => {
        ...
        if (role && role.roleType && role.roleName && Utils().parseJWT()) {
          if (
            Utils().parseJWT()[role.roleType] === role.roleName ||
  ...
  const getCustomResourcesAllNamespaces = item => {
    if (namespace.current && item.spec.scope === 'Namespaced')
      return getCustomResources(item, namespace.current).catch(error =>
        handleError(error)
      );
    else
      return apiManager.current
      ...
      if (!type) {
        if (abortWatch(watch.plural)) {
          ...
          if (
            ...
            watches.current = watches.current.filter(item => {
              return item.plural !== watch.plural;
            });
          }
          watchResource(
            watch.api,
  ...
  const abortWatch = watch => {
    ...
    if (
      ...
      watch === 'dashboardconfigs/'
    )
      return true;

    let w = watches.current.find(item => {
      return item.plural === watch;
    });
    if (w) {
    ...
    if (type === 'ADDED' || type === 'MODIFIED') {
      ...
      if (CRD) {
        if (CRD.metadata.resourceVersion !== object.metadata.resourceVersion) {
          CRDs.current = CRDs.current.filter(item => {
            return item.metadata.name !== CRD.metadata.name;
          });
          CRDs.current.push(object);
          CRDs.current.sort((a, b) =>
  ...
  const manageCallbackCRDs = (object, type) => {
    ...
    if (sidebarCallback.current)
      sidebarCallback.current(
        CRDs.current.filter(item => {
          return (
            item.metadata.annotations && item.metadata.annotations.favourite
          );
        })
      );
  };
    ...
    if (type === 'ADDED' || type === 'MODIFIED') {
      ...
      if (CV) {
        if (CV.metadata.resourceVersion !== object.metadata.resourceVersion) {
          customViews.current = customViews.current.filter(item => {
            return item.metadata.name !== CV.metadata.name;
          });
          customViews.current.push(object);
          customViews.current.sort((a, b) =>
            a.metadata.name.localeCompare(b.metadata.name)
          );
  ...
  const getPODsAllNamespaces = fieldSelector => {
    if (namespace.current)
      return getPODs(namespace.current, fieldSelector).catch(error =>
        handleError(error)
      );
    else
      return apiManager.current
        .getPODsAllNamespaces(fieldSelector)
        .catch(error => handleError(error));
  };
  ...
  const getConfigMaps = (_namespace, fieldSelector) => {
    return apiManager.current
      .getConfigMaps(_namespace, fieldSelector)
      .catch(error => handleError(error));
  };

  const getApis = () => {
    return apiManager.current.getApis().catch(error => handleError(error));
  };

  const getGenericResource = (
    partialPath,
    setItems,
    noNamespaceCheck,
    notifyFunc
