...
export default function ApiManager() {
  ...
  const getCustomResourcesAllNamespaces = item => {
    return fetch(
      'http://localhost:3001/clustercustomobject/' + item.spec.names.plural
    ).then(res => res.json());
  };
  ...
  const getCustomResources = (item, namespace) => {
    return fetch(
      ...
      .then(res => res.json())
      .then(res => {
        res.body.items = res.body.items.filter(
  ...
  const deleteCustomResource = (group, version, namespace, plural, name) => {
    return fetch('http://localhost:3001/clustercustomobject/' + plural, {
      method: 'DELETE'
    })
      .then(res => res.json())
      .then(res => {
        watchList.forEach(w => {
          if (w.plural === plural) w.callback('DELETED', res);
        });
      });
  };
    ...
    return fetch('http://localhost:3001/clustercustomobject/' + plural, {
      ...
      .then(res => {
        watchList.forEach(w => {
          ...
          } else if (w.plural === plural) w.callback('ADDED', res.body);
        });
        return res;
      });
  };
      ...
      return fetch('http://localhost:3001/clustercustomobject/' + plural, {
        ...
        .then(res => {
          if (plural === 'views') {
            ...
            if (itemDC.spec.resources) {
              itemDC.spec.resources.forEach(item => {
                ...
                if (item.render && item.render.tabs) {
                  item.render.tabs = item.render.tabs.filter(
                    tab => tab !== null
                  );
                }
                return item;
            ...
            watchList.forEach(w => {
              if (w.plural === plural) w.callback('MODIFIED', res.body);
            });
          }
          return res;
        });
  ...
  const updateCustomResourceDefinition = (name, item) => {
    ...
    itemDC.metadata.resourceVersion++;
    watchList
      .find(item => item.plural === 'customresourcedefinitions/')
      .callback('MODIFIED', itemDC);
  ...
  const watchFunction = (path, callback, done) => {
    ...
    if (array[4] === 'views' && path.slice(-1) === '/') array[4] = 'views/';
  ...
  const fetchMetrics = path => {
    let array = path.split('/');
    if (array[4] === 'nodes') return getMetricsNodes();
  ...
  const getConfigMaps = namespace => {
    return fetch('http://localhost:3001/configmaps/' + namespace).then(res =>
      res.json()
    );
  };

  const sendDeletedSignal = (plural, CRD) => {
    watchList.find(item => item.plural === plural).callback('DELETED', CRD);
  };

  const sendAddedSignal = (plural, CRD) => {
    watchList.find(item => item.plural === plural).callback('ADDED', CRD);
  };

  const sendModifiedSignal = (plural, CRD) => {
    watchList.find(item => item.plural === plural).callback('MODIFIED', CRD);
  };

  const sendAbortedConnectionSignal = plural => {
    watchList.find(item => item.plural === plural).done();
  };
  ...
  const fetchRaw = (path, method, item) => {
    ...
    return fetch(path, requestOptions)
      ...
      .then(res => {
        ...
        let plural;
        if (array[1] === 'api') {
          if (array[3] === 'namespaces') plural = array[5];
          else plural = array[4];
        } else {
          if (array[3] === 'namespaces') plural = array[4];
          else plural = array[4];
        }
