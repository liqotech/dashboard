...
function FormGenerator(props) {
  const util = Utils();
  let schema = util.OAPIV3toJSONSchema(
  ...
  let [currentSpec, setCurrentSpec] = useState(() =>
    props.onUpdate ? props.CR.spec : null
  );
  ...
  let metadata = {
    ...
    properties: {
      name: {
        type: 'string',
        description:
          'The name of the resource. It is a string that uniquely identifies this object within the current namespace (if the resource is namespaced).'
      }
    }
  };
  ...
  if (props.CRD.spec.scope !== 'Cluster') {
    metadata.properties.namespace = {
      type: 'string',
      description:
        "The namespace of the resource. A namespace is a DNS compatible label that objects are subdivided into. The default namespace is 'default'. "
    };
  }
      ...
      {!props.onUpdate ? (
        ...
        >
          <Form
            ...
            widgets={widgets}
            onChange={value => {
              setCurrentMetadata(value.formData);
            }}
          >
            <div />
          </Form>
