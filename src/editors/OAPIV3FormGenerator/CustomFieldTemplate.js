...
export function checkChildren(props) {
  ...
  Object.keys(props.schema.properties).forEach(item => {
    if (
      props.schema.properties[item].type === 'object' ||
...
function customObject(props) {
  return (
    ...
    >
      <Collapse
        defaultActiveKey={props.id === 'root' ? 'collapse_' + props.id : null}
      >
        <Collapse.Panel
            ...
            <Col span={10}>
              <div>
                <Input
                  ...
                  value={props.label}
                  onChange={({ target }) => {
                    if (!props.readonly) props.onKeyChange(target.value);
                  }}
                />
              </div>
            </Col>
