...
function ReferenceHandler(props) {
  ...
  useEffect(() => {
    CRD.current = window.api.getCRDFromKind(
      props.label.split('/')[1].slice(0, -3)
    );
    ...
    if (onViewer || selectedNS === 'all namespaces') {
      window.api.getCustomResourcesAllNamespaces(CRD.current).then(res => {
        if (!onViewer) {
          ...
          CROptions.current = [];
          CRs.current.forEach(CR =>
            CROptions.current.push(
              <Select.Option
          ...
          CR.current = res.body.items.find(item => {
            return (
              item.metadata.name ===
              props.children.props.children[0].props.formData.name
            );
          });
  ...
  const handleChangeCR = items => {
    ...
    props.children.props.children[0].props.onChange({
      name: items,
      namespace: CRs.current.find(CR => (CR.metadata.name = items)).metadata
        .namespace
    });
  };
  ...
  return window.api.getCRDFromKind(props.label.split('/')[1].slice(0, -3)) ? (
    <div>
      <div>
                  ...
                  <>
                    {onViewer ? (
                      <>
                        <Col span={13}>
                          <Input
                            defaultValue={CR.current.metadata.name}
                            value={CR.current.metadata.name}
            ...
            <Row align="middle">
              <Col span={24}>
                <Alert
                  message={
                    'No resource ' +
                    props.children.props.children[0].props.formData.kind +
                    ' found.'
                  }
                  type="warning"
                  showIcon
