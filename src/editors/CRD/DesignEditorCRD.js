...
function DesignEditorCRD(props) {
  ...
  const previewFunction = CRTemplate => {
    if (CRTemplate === 'default' && props.CR[0]) {
      ...
      if (chosenTemplate.spec.names.kind === 'PieChart') {
        setPreview(
          <div>
            ...
            <br />
            <PieChart CR={props.CR[0].spec} template={CRTemplate} />
          </div>
        );
        ...
        setPreview(
          <div>
            ...
            <br />
            <HistoChart CR={props.CR[0].spec} template={CRTemplate} />
          </div>
        );
      } else if (chosenTemplate.spec.names.kind === 'Graph') {
        setPreview(
          <GraphNet
            customResources={props.CR}
            template={CRTemplate}
            CRD={props.CRD}
      ...
      setPreview(
        <Empty
          key={'empty_preview'}
          description={<strong>No resource to show</strong>}
        />
      );
    }
  };
  ...
  const onClick_save = () => {
    if (CRChosenTemplate !== 'default') {
      props.CRD.metadata.annotations.template =
      ...
      window.api
        ...
        .then(() => {
          window.api
            .updateCustomResourceDefinition(props.CRD.metadata.name, props.CRD)
            .then(() => {
      ...
      <div style={{ marginBottom: 100 }}>
        <Tabs type="card" animated activeKey={currentTab} onChange={changeTab}>
          <Tabs.TabPane tab={'Template'} key={'0'}>
            <Card bordered={false}>{options}</Card>
          </Tabs.TabPane>
