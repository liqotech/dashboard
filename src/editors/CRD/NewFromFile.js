...
export default function NewFromFile(props) {
  ...
  useEffect(() => {
    getNamespaced(props.genericResource.metadata.selfLink).then(res =>
      setNamespaced(res)
    );
  }, []);
      ...
      if (key !== 'key' && key !== 'uploaded')
        setColumnHeaders(prev => {
          prev.push({
            ...
            ...getColumnSearchProps(key, text => <div>{text}</div>)
          });
          return [...prev];
        });
    });
    ...
    reader.onload = e => {
      Utils()
        ...
        .then(res => {
           ...
           */
          if (window.customCSVParser) res = window.customCSVParser(res);
  ...
  const uploadFile = () => {
    ...
    if (selectedContent.length > 0) content = selectedContent;
      ...
      {namespaced ? (
        <Row align={'middle'} gutter={[20, 20]}>
          <Col>
            ...
            <Tooltip
              placement="top"
              title={'Select the namespace for the resources'}
            >
