...
function GraphNet(props) {
  ...
  const createNetworkFromTemplate = () => {
    ...
    props.customResources.forEach(item => {
      ...
      if (props.template.spec.group.cluster) {
        ...
        if (
          props.template.spec.group.cluster !==
          utils.index(item.spec, props.template.spec.group.type)
        )
          level = 1;
      }
  ...
  const createNetwork = counter => {
    ...
    props.showRef.forEach(key => {
      if (key) {
        props.onRef[key].forEach(item => {
          if (
            !kinds.current.find(kind => kind === item.kind + '_' + item.level)
          ) {
            kinds.current.push(item.kind + '_' + item.level);
          }
          if (
            !nodes.find(
              node => node.id === item.kind + '_' + item.metadata.name
            )
          ) {
            nodes.push({
    ...
    if (
      !(
        Utils().arraysEqual(graph.nodes, _graph.nodes) &&
  ...
  useEffect(() => {
    if (props.onRef) createNetwork(0);
    else createNetworkFromTemplate();
        ...
        if (event.nodes.length === 1) {
          if (network.isCluster(event.nodes[0]) === true) {
            ...
            let clusterOptionsByData = {
              joinCondition: function (childOptions) {
                return (
                  childOptions.group === node.group && childOptions.level !== 0
                );
              },
              clusterNodeProperties: {
