...
function ConnectedPeer(props) {
  ...
  const calculateIncomingMetricsPods = (home, po, res) => {
    ...
    let totalRAMPercentage = home.totalPodsRAM / (home.totalMemory * props.config.spec.advertisementConfig.outgoingConfig.resourceSharingPercentage / 100) * 100;
    let totalCPUPercentage = home.totalPodsCPU / (home.totalCPU * props.config.spec.advertisementConfig.outgoingConfig.resourceSharingPercentage / 100) * 100;

    let totalAvailableRAMPercentage = home.totalAllocatablePodsRAM / (home.totalMemory * props.config.spec.advertisementConfig.outgoingConfig.resourceSharingPercentage / 100) * 100;
    let totalAvailableCPUPercentage = home.totalAllocatablePodsCPU / (home.totalCPU * props.config.spec.advertisementConfig.outgoingConfig.resourceSharingPercentage / 100) * 100;
  ...
  const menu = (
    <Menu>
      ...
      <Menu.Item danger key={'disconnect'} icon={<CloseOutlined />} onClick={() => {disconnect()}}>
        Disconnect
      </Menu.Item>
    </Menu>
  )
  ...
  if(props.client)
    clientPercent = clientPercentage();

  /** Quantity of available resources you are using on me */
  let serverPercent = 0;

  if(props.server)
    serverPercent = serverPercentage();
  ...
  return(
    <div style={{ paddingTop: '1em', paddingBottom: '1em' }}>
        ...
        <Collapse.Panel header={
          <>
            <Space size={'middle'}>
              <div>
                <Tooltip title={
                  <div>You {props.server ? ': ' + serverPercent + '%' : null}</div>
                } placement={'top'}>
                  <Progress
                    width={54}
                    type="circle"
                    ...
                    strokeWidth={10}
                    trailColor={(props.server && (sharing.client || sharing.server)) ? null : backgroundColor}
                    strokeColor={(props.server && (sharing.client || sharing.server)) ? getColor(serverPercent) : backgroundColor}
                    format={() => (
                      <Avatar
                        size="large"
                        style={sharing.client || sharing.server ? { backgroundColor: '#1890ff' } : { backgroundColor: '#ccc' }}
                        icon={<UserOutlined />}
                      />
                    )}
                  />
              ...
              <div>
                <Tooltip title={props.client ? clientPercent + '%' : null} placement={'top'}>
                  <Progress
                    ...
                    strokeWidth={10}
                    trailColor={(props.client && (sharing.client || sharing.server)) ? null : backgroundColor}
                    strokeColor={(props.client && (sharing.client || sharing.server)) ? getColor(clientPercent) : backgroundColor}
                    format={() => (
                      <Avatar
                        size="large"
                        style={sharing.client || sharing.server ? { backgroundColor: '#1890ff' } : { backgroundColor: '#ccc' }}
            ...
            <Row align={'middle'}>
              <Col flex={2}>
                <Typography.Text type={'secondary'}>
                  ...
                  {  sharing.client || sharing.server ? ', sharing' : ', not sharing'}
                </Typography.Text>
              </Col>
              <Col flex={3}>
      ...
      {getPeerProperties(props.client, props.server, props, showProperties, setShowProperties)}
