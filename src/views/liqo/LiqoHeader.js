...
function LiqoHeader(props) {
  ...
  const [clusterName, setClusterName] = useState(
    props.config ? props.config.spec.discoveryConfig.clusterName : null
  );
  const [onEdit, setOnEdit] = useState(false);

  useEffect(() => {
    window.api
      .getConfigMaps(LIQO_NAMESPACE, 'metadata.name=cluster-id')
      .then(res => {
  ...
  const selectMode = (
    <Select defaultValue={'Autonomous'}>
      <Select.Option value="Autonomous">
        <Tooltip
          placement={'left'}
          title={'Let LIQO decide who to share resources with'}
        >
          Autonomous
        </Tooltip>
      </Select.Option>
      <Select.Option value="Tethered">
        <Tooltip
          placement={'left'}
          title={'Choose to connect to a single foreign LIQO peer'}
        >
          Tethered
        </Tooltip>
      </Select.Option>
          ...
          <div>
            {onEdit ? (
              <Input
                ...
                autoFocus
                onChange={value => {
                  setClusterName(value.target.value);
                }}
