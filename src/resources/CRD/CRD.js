...
function CRD(props) {
  ...
  useEffect(() => {
    if (!_.isEmpty(CRD)) loadCustomResources();
  }, [CRD]);
  ...
  const loadCustomResources = () => {
    setCRD(prev => {
      ...
      window.api
        .getCustomResourcesAllNamespaces(prev)
        .then(res => {
  ...
  const handleClick_fav = async () => {
    ...
    await window.api.updateCustomResourceDefinition(CRD.metadata.name, CRD);
  };
      ...
      <div>
        <div>
              ...
              >
                <Tooltip title={CRD.metadata.name} placement={'top'}>
                  <Typography.Text strong style={{ fontSize: 20 }}>
                    {props.altName ? props.altName : CRD.spec.names.kind}
                  </Typography.Text>
                </Tooltip>
              </Link>
            }
          ...
          <Rate
            ...
            count={1}
            value={
              CRD.metadata.annotations && CRD.metadata.annotations.favourite
                ? 1
                : 0
            }
          ...
          <Descriptions style={{ marginTop: 15, marginLeft: 15 }}>
            <Descriptions.Item>
              <Typography.Paragraph
                editable={{ onChange: editDescription }}
                type={'secondary'}
  ...
  const findTemplate = _CRD => {
    if (
      (_CRD.metadata.annotations && _CRD.metadata.annotations.template) ||
      props.altTemplate
    ) {
      let CR = null;
      let array = [];
  ...
  const paginationSizeChange = (current, size) => {
    if (size > pageSize.current)
      currentPageSizeChange.current = Math.ceil(
        (pageSize.current * current) / size
      );
    else
      currentPageSizeChange.current = Math.floor(
        (pageSize.current * (current - 1)) / size + 1
      );
