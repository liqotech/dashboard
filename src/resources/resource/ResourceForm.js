...
export default function ResourceForm(props) {
  const [currentTab, setCurrentTab] = useState(
    props.currentTab ? props.currentTab : 'metadata'
  );
  ...
  useEffect(() => {
    ...
    Object.keys(props.resource).forEach(key => {
      if (
        typeof props.resource[key] === 'object' &&
        props.resource[key] !== null
      ) {
        setTabList(prev => [
          ...prev,
          {
        ...
        setContentList(prev => {
          let readonly =
            key === 'metadata' || key === 'status' || props.readonly;
          return {
            ...prev,
            [key]: (
              <div
                key={
                  key + '_' + props.resourceName
                    ? props.resourceName
  ...
  const searchProperty = (value, tab) => {
    ...
    setContentList(prev => {
      prev[tab] = (
        <div
          key={
            tab + '_' + props.resourceName
              ? props.resourceName
              : props.resource.metadata.name + '#' + value
          }
        >
          <Alert.ErrorBoundary>
            <div aria-label={'form_' + tab}>
              <FormViewer
                {...props}
                resource={JSON.parse(JSON.stringify(searchedRes))}
                show={'form'}
                resourceName={
                  props.resourceName
                    ? props.resourceName
                ...
                origResource={
                  props.origResource ? props.origResource : props.resource
                }
                resourceNamespace={
                  props.resourceNamespace
                    ? props.resourceNamespace
