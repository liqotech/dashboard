...
export default function CustomTab(props) {
  ...
  useEffect(() => {
    setCardList(
      props.content.map(x => {
        ...
        if (!tmp) {
          tmp = cardList.find(item =>
            _.isEqual(item.cardContent, x.cardContent)
          );
          if (tmp) {
            layoutRef.current.find(item => item.i === tmp.cardTitle).i =
              x.cardTitle;
            cardListResized.current.find(
              item => item.cardTitle === tmp.cardTitle
  ...
  const onDeleteContent = cardName => {
    ...
    tempResourceConfig.render.tabs.find(
      item => item.tabTitle === props.tabTitle
    ).tabContent = tempResourceConfig.render.tabs
  ...
  const onAddContent = kind => {
    let key = cardList.filter(item =>
      item.cardTitle.includes('New ' + kind + ' #')
    ).length;
    let newCard = {
      cardContent: null,
      cardDisplay: kind,
  ...
  const onLayoutChange = (layout, layouts) => {
     ...
     * by the props change
     */
    if (layout.length !== props.content.length) return;
    ...
    let content = props.content.map(x => {
      return {
        ...x,
        autoSize: cardList.find(item => item.cardTitle === x.cardTitle).autoSize
      };
    });
      ...
      if (layouts) {
        ...
        tempResourceConfig.render.tabs
          ...
          .tabContent.forEach(card => {
            let newCard = content.find(
              item => item.cardTitle === card.cardTitle
            );
            if (!newCard.autoSize) card.cardLayout = newCard.cardLayout;
          });
        updateResourceConfig(tempResourceConfig, params, location);
      }
    } else if (!layouts) {
  ...
  const onResize = card => {
    if (
      ...
      cardListResized.current.find(item => item.cardTitle === card.cardTitle)
        .cardLayout.h !== card.cardLayout.h
    ) {
      layoutRef.current.find(
        item => item.i === card.cardTitle
      ).h = cardListResized.current.find(
        item => item.cardTitle === card.cardTitle
      ).cardLayout.h;
      layoutRef.current.find(
        item => item.i === card.cardTitle
  ...
  cardList.forEach(card => {
    if (
      ...
      card.cardLayout.h !==
        props.content.find(item => item.cardTitle === card.cardTitle).cardLayout
          .h
    ) {
      card.cardLayout = props.content.find(
        item => item.cardTitle === card.cardTitle
      ).cardLayout;
      layoutRef.current.find(
        item => item.i === card.cardTitle
