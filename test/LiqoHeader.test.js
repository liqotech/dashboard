...
function mocks(error, errorCM) {
  fetch.mockResponse(req => {
    if (
      ...
      if (req.method === 'PUT') {
        if (!error) {
          ...
          return Promise.resolve(
            new Response(JSON.stringify({ body: config }))
          );
      ...
      if (errorCM) return Promise.reject(Error500.body);
      else
        return Promise.resolve(
...
async function changeClusterName() {
  ...
  const clusterID = await screen.findByText(
    '10e3e821-8194-4fb1-856f-d917d2fc54c0'
  );
...
describe('LiqoHeader', () => {
  ...
  test(
    'The header shows the cluster name and cluster ID',
    async () => {
      mocks();
