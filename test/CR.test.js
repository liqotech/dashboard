...
async function setup(adv) {
  ...
  window.api.getCRDs().then(async () => {
    ...
    if (adv) {
      ...
      render(
        <MemoryRouter>
          <CR cr={a.body.items[0]} crd={adv_crd} />
        </MemoryRouter>
      );
    } else {
      render(
        <MemoryRouter>
          <CR cr={l.body.items[0]} crd={liqo_crd} template={p.body.items[0]} />
        </MemoryRouter>
      );
    }
  });
...
describe('CR', () => {
  ...
  test(
    'Additional properties are showed in the CR',
    async () => {
      mockFetch();
      await setup(true);
  ...
  test(
    'CR JSON show no spec or status when there is none',
    async () => {
      fetch.mockImplementation(url => {
        if (url === 'http://localhost:3001/customresourcedefinition') {
  ...
  test(
    'CR delete popup show on click and resource is deleted',
    async () => {
      mockFetch();
      await setup();
