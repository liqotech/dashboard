...
jest.mock('./src/themes/ThemeModifier', () => {
  return function ThemeModifier() {
    return <div />;
  };
});

jest.mock('./src/themes/ThemeUploader', () => {
  return function ThemeUploader() {
    return <div />;
  };
});
...
jest.mock('react-ace', () => {
  return ({ onChange }) => {
    return (
      <input aria-label={'editor'} onChange={e => onChange(e.target.value)} />
    );
  };
});
...
jest.mock('ace-builds/src-noconflict/mode-markdown', () => {
  return null;
});
jest.mock('ace-builds/src-noconflict/mode-javascript', () => {
  return null;
});
jest.mock('ace-builds/src-noconflict/theme-dawn', () => {
  return null;
});
